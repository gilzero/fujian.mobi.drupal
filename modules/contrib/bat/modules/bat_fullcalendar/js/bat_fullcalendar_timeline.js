...
(function (Drupal, $, once) {
  Drupal.behaviors.bat_event = {
    attach: function (context) {
      ...
      const elements = once('bat_event', '#calendar', context);
      const key=0;
      elements.forEach(function (element) {
        $(element).fullCalendar({
          ...
          height: drupalSettings.batCalendar[key].calendarHeight ? drupalSettings.batCalendar[key].calendarHeight : 300,
          ...
          eventClick: function (event, jsEvent, view) {
            ...
            if (event.editable) {
              ...
              console.log("dentro editable");
              console.log(event);
              var unit_id = unitIdFromResource(event.resourceId);
          ...
          select: function (start, end, jsEvent, view, resource) {
            if (resource.create_event) {
              ...
              var unit_id = unitIdFromResource(resource.id);
  ...
  function unitIdFromResource(resourceId) {
    var unitId = resourceId.substring(1);
    return unitId;
  }
