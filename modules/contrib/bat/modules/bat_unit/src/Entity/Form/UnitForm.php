...
class UnitForm extends ContentEntityForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['meta'] = [
      ...
      'published' => [
        '#type' => 'html_tag',
        '#tag' => 'h3',
        '#value' => $entity->getStatus() ? $this->t('Published') : $this->t('Not published'),
        '#access' => !$entity->isNew(),
        '#attributes' => [
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Unit.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Unit.', [
          '%label' => $entity->label(),
        ]));
    }
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    if ($entity->isNew()) {
      ...
      $element['publish']['#value'] = $entity->getStatus() ? $this->t('Save and keep published') : $this->t('Save and publish');
    }
    ...
    if ($entity->isNew()) {
      ...
      $element['unpublish']['#value'] = !$entity->getStatus() ? $this->t('Save and keep unpublished') : $this->t('Save and unpublish');
    }
